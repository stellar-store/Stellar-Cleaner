...
CleanerPage::CleanerPage(QWidget *parent) : QWidget(parent) {
    m_tempCleaner    = new TempCleaner(this);
    m_browserCleaner = new BrowserCleaner(this);
    m_logCleaner     = new LogCleaner(this);
    m_pkgCleaner     = new PackageCleaner(this);
    m_privCleaner    = new PrivacyCleaner(this);
    m_trashCleaner   = new TrashCleaner(this);

    m_cleaners = { m_tempCleaner, m_browserCleaner, m_logCleaner,
                   m_pkgCleaner,  m_privCleaner,    m_trashCleaner };

    for (auto *c : m_cleaners) connectCleaner(c);
...
void CleanerPage::connectCleaner(BaseCleaner *c) {
    connect(c, &BaseCleaner::logMessage, this, &CleanerPage::appendLog);
    connect(c, &BaseCleaner::analyzeProgress, this, [this](int pct, const QString &item) {
        m_progress->setValue(pct);
        m_lblStatus->setText(QString("Analizando: %1").arg(item));
    });
    connect(c, &BaseCleaner::cleanProgress, this, [this](int pct, const QString &item) {
        m_progress->setValue(pct);
        m_lblStatus->setText(item);
    });
}

void CleanerPage::setupUI() {
    ...
    auto *titleLbl = new QLabel("Limpiador del Sistema", toolbar);
    titleLbl->setObjectName("pageTitle");
    tbLay->addWidget(titleLbl);

    auto *subtitleLbl = new QLabel("  Selecciona categorías y ejecuta el análisis", toolbar);
    ...
    m_btnClean = new QPushButton("🗑  Limpiar Ahora", toolbar);
    ...
    connect(btnSelAll,  &QPushButton::clicked, this, &CleanerPage::onSelectAll);
    connect(btnSelNone, &QPushButton::clicked, this, &CleanerPage::onSelectNone);
    connect(m_btnAnalyze, &QPushButton::clicked, this, &CleanerPage::onAnalyze);
    connect(m_btnClean,   &QPushButton::clicked, this, &CleanerPage::onClean);
    ...
    m_tree->expandAll();
    connect(m_tree, &QTreeWidget::itemChanged, this, &CleanerPage::onItemChanged);
...
void CleanerPage::onAnalyze() {
    ...
    m_currentCleanerIdx = 0;

    // Run all cleaners in sequence using QtConcurrent
    connect(m_watcher, &QFutureWatcher<void>::finished,
            this, &CleanerPage::onAnalyzeFinished, Qt::UniqueConnection);

    auto future = QtConcurrent::run([this]() {
        for (int i = 0; i < m_cleaners.size(); ++i) {
            QMetaObject::invokeMethod(this, "appendLog", Qt::QueuedConnection,
                Q_ARG(QString, QString("[Analyze] %1...").arg(m_cleaners[i]->name())));
            m_cleaners[i]->analyze();
            int globalPct = ((i + 1) * 100) / m_cleaners.size();
            QMetaObject::invokeMethod(m_progress, "setValue",
                Qt::QueuedConnection, Q_ARG(int, globalPct));
        }
    });
    m_watcher->setFuture(future);
}

void CleanerPage::onAnalyzeFinished() {
    m_analyzing = false;
    m_btnAnalyze->setEnabled(true);
    m_btnClean->setEnabled(true);
    m_lblStatus->setText("Análisis completo. Listo para limpiar.");
...
void CleanerPage::onClean() {
    if (m_analyzing || m_cleaning) return;

    // Collect selected entries per cleaner
    QMap<BaseCleaner*, QList<CleanEntry>> toClean;
    qint64 totalSelected = 0;

    for (int i = 0; i < m_tree->topLevelItemCount(); ++i) {
        auto *catItem = m_tree->topLevelItem(i);
        BaseCleaner *cleaner = m_cleaners[i];
        QList<CleanEntry> sel;
        for (const auto &e : cleaner->entries()) {
        ...
        if (!sel.isEmpty()) toClean[cleaner] = sel;
    }
    ...
    QString confirmMsg = QString(
        "¿Estás seguro de que deseas limpiar los elementos seleccionados?\n\n"
        "Se liberarán aproximadamente %1 de espacio.\n\n"
        "Esta acción NO se puede deshacer.")
        .arg(FileUtils::formatSize(totalSelected));

    if (QMessageBox::question(this, "Confirmar Limpieza", confirmMsg,
    ...
    m_btnClean->setEnabled(false);
    m_progress->setValue(0);
    appendLog(QString("[%1] === Limpieza iniciada ===").arg(
        QDateTime::currentDateTime().toString("hh:mm:ss")));

    connect(m_watcher, &QFutureWatcher<void>::finished,
            this, &CleanerPage::onCleanFinished, Qt::UniqueConnection);

    auto future = QtConcurrent::run([this, toClean]() mutable {
        int idx = 0;
        for (auto it = toClean.begin(); it != toClean.end(); ++it, ++idx) {
            qint64 freed = it.key()->clean(it.value());
            m_totalFreed += freed;
            int pct = ((idx+1) * 100) / toClean.size();
            QMetaObject::invokeMethod(m_progress, "setValue",
                Qt::QueuedConnection, Q_ARG(int, pct));
            QMetaObject::invokeMethod(this, "appendLog", Qt::QueuedConnection,
                Q_ARG(QString, QString("[Clean] %1: liberados %2")
                    .arg(it.key()->name())
...
void CleanerPage::onCleanFinished() {
    m_cleaning = false;
    m_btnAnalyze->setEnabled(true);
    m_btnClean->setEnabled(false);
    m_progress->setValue(100);
    m_lblStatus->setText(QString("✅ Limpieza completa — Liberados %1")
        .arg(FileUtils::formatSize(m_totalFreed)));
    appendLog(QString("[%1] === Limpieza completa: %2 liberados ===")
        .arg(QDateTime::currentDateTime().toString("hh:mm:ss"))
        .arg(FileUtils::formatSize(m_totalFreed)));
    emit totalFreedChanged(m_totalFreed);

    QMessageBox::information(this, "Limpieza Completada",
...
void CleanerPage::onItemChanged(QTreeWidgetItem *item, int col) {
...
void CleanerPage::updateSummary() {
    ...
    for (auto *cleaner : m_cleaners) {
        for (const auto &e : cleaner->entries()) {
            ...
            for (int i = 0; i < m_tree->topLevelItemCount(); ++i) {
                auto *catItem = m_tree->topLevelItem(i);
                if (m_cleaners[i] != cleaner) continue;
                for (int j = 0; j < catItem->childCount(); ++j) {
                    auto *child = catItem->child(j);
                    if (child->data(COL_NAME, Qt::UserRole+1).toString() == e.id &&
                        child->checkState(COL_NAME) == Qt::Checked) {
...
void CleanerPage::appendLog(const QString &msg) {
