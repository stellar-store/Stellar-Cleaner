...
KernelPage::KernelPage(QWidget *parent) : QWidget(parent) {
    ...
    m_cleaners = { m_kernelCleaner };

    connect(m_kernelCleaner, &BaseCleaner::logMessage, this, &KernelPage::appendLog);
    connect(m_kernelCleaner, &BaseCleaner::analyzeProgress, this, [this](int pct, const QString &item) {
        m_progress->setValue(pct);
        m_lblStatus->setText(QString("Analizando: %1").arg(item));
    });
    connect(m_kernelCleaner, &BaseCleaner::cleanProgress, this, [this](int pct, const QString &item) {
        m_progress->setValue(pct);
...
void KernelPage::setupUI() {
    ...
    auto *titleLbl = new QLabel("🔧 Limpiador del Kernel", toolbar);
    titleLbl->setObjectName("pageTitle");
    tbLay->addWidget(titleLbl);

    auto *subtitleLbl = new QLabel("  Optimiza y limpia archivos del kernel", toolbar);
    ...
    connect(btnSelAll,  &QPushButton::clicked, this, &KernelPage::onSelectAll);
    connect(btnSelNone, &QPushButton::clicked, this, &KernelPage::onSelectNone);
    connect(m_btnAnalyze, &QPushButton::clicked, this, &KernelPage::onAnalyze);
    connect(m_btnClean,   &QPushButton::clicked, this, &KernelPage::onClean);
    ...
    m_tree->expandAll();
    connect(m_tree, &QTreeWidget::itemChanged, this, &KernelPage::onItemChanged);
...
void KernelPage::onAnalyze() {
    ...
    connect(m_watcher, &QFutureWatcher<void>::finished,
            this, &KernelPage::onAnalyzeFinished, Qt::UniqueConnection);

    auto future = QtConcurrent::run([this]() {
        m_kernelCleaner->analyze();
        QMetaObject::invokeMethod(m_progress, "setValue", Qt::QueuedConnection, Q_ARG(int, 100));
    });
    m_watcher->setFuture(future);
}

void KernelPage::onAnalyzeFinished() {
    m_analyzing = false;
...
void KernelPage::onClean() {
    ...
    for (const auto &e : m_kernelCleaner->entries()) {
    ...
    if (toClean.isEmpty()) {
        QMessageBox::information(this, "Nada seleccionado",
            "Selecciona al menos un elemento para limpiar.");
        return;
    }
    ...
    connect(m_watcher, &QFutureWatcher<void>::finished,
            this, &KernelPage::onCleanFinished, Qt::UniqueConnection);

    auto future = QtConcurrent::run([this, toClean]() {
        qint64 freed = m_kernelCleaner->clean(toClean);
        m_totalFreed += freed;
        QMetaObject::invokeMethod(m_progress, "setValue", Qt::QueuedConnection, Q_ARG(int, 100));
        QMetaObject::invokeMethod(this, "appendLog", Qt::QueuedConnection,
            Q_ARG(QString, QString("[Clean] Kernel Cleaner: liberados %1")
                .arg(FileUtils::formatSize(freed))));
    });
    m_watcher->setFuture(future);
}

void KernelPage::onCleanFinished() {
    m_cleaning = false;
    m_btnAnalyze->setEnabled(true);
    m_btnClean->setEnabled(false);
...
void KernelPage::onItemChanged(QTreeWidgetItem *item, int col) {
...
void KernelPage::appendLog(const QString &msg) {
